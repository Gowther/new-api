...
func (p *LinuxDOProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	// Build the redirect URI from the configured server address like the other
	// providers; deriving it from the request Host header would let callers
	// control where the provider returns the authorization code.
	redirectURI := fmt.Sprintf("%s/oauth/linuxdo", system_setting.ServerAddress)
