...
const modelVendorGroupsMatchModels = (groups, models) => {
  const groupedModels = (Array.isArray(groups) ? groups : []).flatMap(
    (group) => (Array.isArray(group.models) ? group.models : []),
  );
...
const EditChannelModal = (props) => {
  ...
  const confirmUncoveredModelRules = (uncoveredModels) =>
    new Promise((resolve) => {
      const modal = Modal.confirm({
        title: t('模型未被模型管理规则覆盖'),
        content: (
          <div className='text-sm leading-6'>
            <div>
              {t(
                '当前渠道里的下列模型没有匹配任何已启用的模型管理规则，可能不会在模型广场展示或缺少元数据：',
              )}
            </div>
            <div className='font-mono text-xs break-all text-orange-600 mt-1'>
              {uncoveredModels.join(', ')}
            </div>
            <div className='mt-2'>
              {t(
                '你可以返回补充模型管理规则，也可以确认这是预期配置后继续保存。',
              )}
            </div>
          </div>
        ),
  ...
  const checkModelRuleCoverageBeforeSubmit = async (models) => {
    try {
      const res = await API.post('/api/models/rule_coverage/check', {
        models,
      });
      const { success, message, data } = res.data;
      if (!success) {
        showError(message || t('模型管理规则检查失败'));
        return false;
      }
      const uncoveredModels = data?.uncovered || [];
      if (uncoveredModels.length === 0) {
        return true;
      }
      const action = await confirmUncoveredModelRules(uncoveredModels);
      return action === 'submit';
    } catch (error) {
      showError(error?.message || t('模型管理规则检查失败'));
      return false;
    }
  };
  ...
  const submit = async () => {
    ...
    const modelRuleCovered = await checkModelRuleCoverageBeforeSubmit(
      localInputs.models,
    );
    if (!modelRuleCovered) {
      return;
              ...
              <div className='space-y-4'>
                ...
                <div className='pb-3 border-b border-gray-100'>
                  ...
                  <Form.InputNumber
                    ...
                    step={1}
                    suffix={t('分钟')}
                    extraText={t('填 0 表示使用全局默认自动测试通道间隔时间')}
                    onChange={(value) =>
                          ...
                          <div className='flex flex-wrap items-center justify-between gap-2'>
                            ...
                            {splitByModelVendor && (
                              <Button
                                ...
                                onClick={() =>
                                  fetchModelVendorGroups(selectedModels)
                                }
                              >
                                {t('刷新分组预览')}
                              </Button>
