...
import { OfficialPriceSyncModal } from './OfficialPriceSync';
...
export default function ModelPricingEditor({
  ...
  const [officialPriceModalVisible, setOfficialPriceModalVisible] =
    useState(false);
  ...
  const handleOfficialPriceApplied = (data) => {
    if (data?.mappings) {
      setOfficialMappings(data.mappings);
    }
    setSelectedModelNames([]);
  };
  ...
  return (
    <>
      <Space vertical align='start' style={{ width: '100%' }}>
        <Space wrap className='mt-2'>
          ...
          <Button
            ...
            onClick={() => setOfficialPriceModalVisible(true)}
            style={isMobile ? { width: '100%' } : undefined}
          >
            {t('官方价格匹配')}
            {selectedModelNames.length > 0
      ...
      <OfficialPriceSyncModal
        visible={officialPriceModalVisible}
        onCancel={() => setOfficialPriceModalVisible(false)}
