...
function TokenBreakdown(props: { log: UsageLog; other: LogOtherData }) {
  ...
  rows.push({
    ...
    value: `${promptTokens.toLocaleString()}${cacheReadRate ? ` (${cacheReadRate})` : ''}`,
  })
  rows.push({
  ...
  return (
    ...
    >
      <div className='w-full max-w-full min-w-0 space-y-2.5 overflow-x-hidden py-1 sm:space-y-3'>
        ...
        {props.isAdmin && other?.admin_info?.reject_reason && (
          <DetailSection
            icon={<AlertTriangle className='size-3.5' aria-hidden='true' />}
            label={t('Reject Reason')}
            variant='danger'
          >
            <p className='text-xs wrap-break-word'>{other.admin_info.reject_reason}</p>
          </DetailSection>
        )}
        ...
        {/* Param override (admin-only: nested under admin_info) */}
        {props.isAdmin &&
          adminInfo?.po &&
          ...
          adminInfo.po.length > 0 && (
            ...
            >
              {adminInfo.po.filter(Boolean).map((line, idx) => {
